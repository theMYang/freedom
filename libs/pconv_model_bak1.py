...
#         self.vgg = self.build_vgg()
    ...
    def build_pconv_unet(self, train_bn=True, lr=0.0002):      
        ...
        #without mask
        def encoder_layer(img_in, mask_in, filters, kernel_size, bn=True):
            conv = Activation('relu')(img_in)
            conv = Conv2D(filters=filters, kernel_size=kernel_size, strides=(1, 1), padding='same')(conv)
            if bn:
                conv = BatchNormalization(name='EncBN'+str(encoder_layer.counter))(conv, training=train_bn)
            conv = Activation('relu')(conv)
            conv = Conv2D(filters=filters, kernel_size=kernel_size, strides=(1, 1), padding='same')(conv)
        ...
        def decoder_layer(img_in, mask_in, e_conv, e_mask, filters, kernel_size, bn=True):
            up_img = UpSampling2D(size=(2,2))(img_in)
            concat_img = Concatenate(axis=3)([e_conv,up_img])
            conv = Activation('relu')(concat_img)
            conv = Conv2D(filters=filters, kernel_size=kernel_size, strides=(1, 1), padding='same')(conv)
            if bn:
                conv = BatchNormalization()(conv)
        ...
        pool1 = AveragePooling2D((2, 2))(e_conv1)
#         e_conv1 = identity_block(e_conv1, (32, 32), 3)

        e_conv2, e_mask2 = encoder_layer(pool1, e_mask1, 64, 3)
        pool2 = AveragePooling2D((2, 2))(e_conv2)
#         e_conv2 = identity_block(e_conv2, (64, 64), 3)
        # e_conv2, e_mask2 = identity_block(e_conv2, e_mask2, 64, 3)

        e_conv3, e_mask3 = encoder_layer(pool2, e_mask2, 128, 3)
        pool3 = AveragePooling2D((2, 2))(e_conv3)
        ...
        d_conv6, d_mask6 = decoder_layer(X, e_mask3, e_conv3, e_mask2, 128, 3)
#         d_conv6 = identity_block(d_conv6, (64, 64), 3)

        d_conv7, d_mask7 = decoder_layer(d_conv6, d_mask6, e_conv2, e_mask1, 64, 3)
