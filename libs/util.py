...
# 混合随机缺失和区域缺失
def mix_mask(height=32, width=32, size=1024, channels=1, smooth_time=0, type='mix', block_size=(32, 32)):
    """Generates a random irregular mask with lines, circles and elipses
       channels: time segment numbers of road net
       smooth_time: no block time segment numbers
    """ 
    ...
    if block_height*block_width+size>height*width:
        raise Exception("too much point")
    ...
    # 添加随机缺失
    # 不放回的在height*width中选取size个元素作为mask的缺失点
    rest_size = size  # 剩余缺失点，防止与区域缺失重合
    while(rest_size>0):
        elements = np.random.choice(height*width, rest_size, replace=False)
        coordinate_map = map(lambda x: np.unravel_index(x, (height, width)), elements)
        def f(x, y):
            if((img[x][y]==0).all()):
                nonlocal rest_size
